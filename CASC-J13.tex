...
\textbf{SUPr}~\cite{TBA} is a saturation-based theorem prover built as a native component of the 
...
The impact was interesting, and data in the \LAST{} report~\cite{Sut26-CASC} showed that the 
ATP systems still needed to improve their proof output.
...
The \LAST{} report noted that final calls to external ATP systems were typically reported with all 
the clauses sent to the external system listed as parents -- the most extreme case listed 72367 
parent clauses that were sent to the external Z3 system.
...
\item[12.] Proofs that make \smalltt{assumption}s must propagate and eventually discharge the 
      assumptions.
\end{packed_enumerate}
Additionally, the granularity measure was added to the results, to track the maximal numbers of 
