...
the paper author.
The three subsection of Section~\ref{Systems} were written by Michael Rawson (hopCoP), 
Simon Guilloud (Lisa's Tableau Tactic), and Christoph Weidenbach (SPASS-SCL-FOL 0.1).
...
The ``FLoC Olympic Games'' hosted HOWMANY competitions in
various areas of logic and reasoning, which provided the \THIS{} entrants
with opportunities to observe and interact with developers of other types
of reasoning tools.
...
\item Advise (by email) on contentious issues that arise before the competition, and that cannot
      be agreeably resolved by the organizers and entrants.
\item With the help of the organizers, adjudicate on the acceptability of the sample proofs and
...
\item Be contactable during the competition to advise on contentious issues that arise and that
      cannot be agreeably resolved by the organizers and entrants.
\item With the help of the organizers, adjudicate on the acceptability of the winners' proofs and
      models.
...
\item Inference steps in proofs may not list parents that are not used in the inference.
\item Proofs should graft in subproofs from external systems.
\item A new performance measure, proof granularity, is recorded and reported.
...
% the winning systems.
Section~\ref{ProoVer} describes the design of the ProoVer competition, presents the results,
and discusses new requirements for proofs in the TPTP format NEED REF.
...
\NUMBEROFENTRANTS{} ATP systems competed in the various divisions of \THIS{}.
The division winners of the previous CASC (\LAST{}) and the Prover9~1109a system are automatically
entered into the FOF demonstration division to provide benchmarks against which progress can be 
judged.
The systems, the divisions in which they were entered, and their entrants, are listed in
Table~\ref{Entrants}.
A division acronym in {\em italics} indicates the system was in the demonstration division.
System descriptions are in these competition proceedings~\cite{Sut24-CASC-J12} and on the \THIS{}
web site.
...
two octa-core Intel(R) Xeon(R) CPU E5-2620 v4 @ 2.10GHz CPUs, with hyperthreading (two 
threads per core),
256GiB memory,
...
HOWMANY In \THIS{} a total of 2071 CPU hours were used in 431 wall clock hours, i.e., 7.2 hours spread
over the 60 CPUs, which allowed the competition to run live in one conference day.
For the problems that were solved, 267 CPU hours were used in 53 wall clock hours.

%--------------------------------------------------------------------------------------------------
\subsection{Problems for the TPTP-based Divisions}
\label{TPTPProblems}

The problems for the THF, FOF, EPR, and UEQ divisions were taken from the latest release of the
...
% The problems had to be all different, as assessed by the competition organizers.
...
In the THF, FOF, FNT, and UEQ divisions a wall clock time limit is imposed for each problem.
The minimal time limit for each problem is 120~s.
...
problems quickly and very few slowly~\cite{SS01,Sut24} -- this phenomenon is evident in the 
performance plots from the competition\footnote{%
...
In \THIS{} a 180~s wall clock time limit was imposed for each problem.
...
CASC ranks the ATP systems at the division level.
...
The systems are ranked at (only) the division level, according to the number of problems solved
...
\item Proofs must be in TPTP format~\cite{Sut07-CSR}.
\item Proofs must be structurally correct:
      ...
      \item Proofs that negate the conjecture must correctly annotate the step as
            \smalltt{status(cth)} and have a a single parent with the role \smalltt{conjecture}.
      \item Inference steps must list exactly those parents that are used in the inference.
...
\item The \textit{state-of-the-art contribution} (SotAC) quantifies the unique abilities of each
      ...
      A system's overall SotAC is the average over the problems it solves but that are not solved 
      by all the systems.
\item The \textit{efficiency} balances the number of problems solved with the time taken.
      ...
      problems it solves, multiplied by the fraction of problems solved.
...
\item The \textit{core usage} measures the extent to which the systems take advantage of multiple
      ...
      usage is over the problems it solves with a raw core usage greater than 1.0.
      ...
      and the number of problems solved with a raw core usage less than 1.0 is also noted.
\item The \textit{granularity} measures the extent to which inference steps combine multiple 
      ...
      overall granularity is the average over the the problems it solves with acceptable proof 
      output.
...
The ATP systems are registered for the competition by the registration deadline, using the
...
The ATP systems are delivered to the competition organizers by the system delivery deadline,
as StarExec \smalltt{.tgz} installation packages.
The installation packages may contain only the components necessary for running the system, i.e.,
not including source code, etc.
The competition organizers install and test the packages on StarExec.
...
The entrants must email a \smalltt{.tgz} file of the source code and any files required for
building the StarExec installation package to the competition organizers by the system delivery
deadline.
...
% executable system to the competition organizers by the system delivery deadline.
% In the demonstration division the entrant specifies whether or not the source code is placed on
% the CASC web site.
...
e.g., separate packages for the TFA and TFN divisions, and separate packages for the FOF, EPR, 
and UEQ divisions.
In some cases multiple packages are combined into one by adding a wrapper that detects the 
SPC of the problem, and then calls the appropriate package's entrypoint.
Requiring the use of a different package depending on the problem type is a coarse level of 
non-automation.
Users of ATP systems can have different intentions, that produce distinct tasks for the systems.
Common examples are theorem proving vs. disproving, and testing for unsatisfiability vs. model 
finding.
The latter is often the implementation of the former.
In \NEXT{} entrants will be restricted to maximally two installation packages, corresponding
to those use cases.
The ATP systems will be responsible for detecting the problem type and running appropriate 
components.
This will improve the level of automation in the ATP systems.
...
The structural checking of proofs in the competition theorem proving divisions -- the THF, 
FOF, and UEQ divisions -- was new in \LAST{}.
It was the first step along a path to full verification of proofs in CASC.
In \LAST{}, for most systems, some of their proofs failed the structural checking,
and it was hoped that the situation would have improved by \THIS{}. 
Sadly, it was not the case.
The result tables show the difference between the number of solutions output
and the number of acceptable solutions output, and some commentary is provided.
Some developers knew in advance that their system's proof output was not acceptable, and 
``number of problems solved with an acceptable solution output'' is set to ``-'' rather than 0.
A full analysis of the reasons for the failures is given in Section~\ref{Acceptable}.
...
Vampire~5.0 won the division, but solved less problems than the \LAST{} winner Vampire~4.9.
The developers thought there should be a minimal difference between the two versions, because
they use the same source code.
They suggested that a different compiler version (or compiler flags) was used to build the 
Vampire~5.0 binary, or that, while unlikely, the randomized strategies in the schedules might 
have resulted in plain bad luck for Vampire~5.0.
It is impressive that Vampire~5.0 was able to solve so many problems, which necessarily include 
more difficult problems, with the lowest average time taken (but again,
Vampire~4.9 outperformed Vampire~5.0 in this respect).
The versions of Vampire up to and including 5.0 have separate binaries for first-order and 
higher-order logics.
According to the Vampire developers the two branches of the source code are being merged so that
for \NEXT{} there will be a single binary for all the competition divisions.

\begin{table*}[tbh]
\caption{THF division results}
\label{THFSummary}
\begin{tabular}{lrrrrrrr@{}l@{}rrrrr}
\hline
...
\hline
Vampire~5.0.1          &  352  &  352   &  6.5& 0.32 & 615      & 3.9       & {\tiny /212} &  67 & 285 &    3 &  31 &  TBA  \\
Zipperp'n~2.1.9999     &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
E~3.5.1                &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
Leo-III~1.8.0          &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
LEO-II~1.7.0           &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
\hline
\multicolumn{13}{c}{Demonstration division} \\
\hline
\textit{Vampire~5.0}   &  354  &  354   &  6.8 &   - & 576      & 4.2       & {\tiny /238} &   70 & 284 &   3 &  31 &  TBA  \\
\hline
\end{tabular}
\end{table*}

Vampire~5.0 has the best results over almost all measures.
The exception is core usage, where Zipperposition solved more problems using multiple cores,
and had the highest core usage.
E solved the most problems before starting to use multiple cores, and had good core usage
when it did use multiple cores.
Leo-III used multiple cores for all the problems it solved.
cvc5's high average time taken is a consequence of it's sequential strategy schedule that uses
a single core (the one result that appears to use 1.5 cores is an artifact of the competition 
measurement harness).
% which occurs when the CPU and wall clock usages are very close).
The problem category rankings are generally aligned with the overall ranking, with the exception
that Zipperposition had better performance on problems without equality.

The detailed results show that E solved eight problems ``at the time limit''.
An examination of E's logs revealed that the problems were actually solved in less time, but the 
proof was printed only when the time limit was reached.
In two cases, one in each problem category, the claim of theoremhood was output only after the
time limit, and thus not counted as solved.
The developer explained: ``E waits for all its children to terminate before printing the proof, 
so if one does not respond to the {\tt SIG\_TERM} termination signal the scheduler hangs.
The scheduler catches {\tt SIG\_TERM} while spawning worker processes, and this behaviour was 
inadvertently inherited by the children. This has hopefully now been remedied.''
% This might have caused them to ignore {\tt SIG\_TERM} in other situations.
% This has hopefully now been remedied by explicitly resetting the signal handler in the children.''
This is another instance of the careful evaluation done in CASC revealing a weakness in an ATP 
system, leading to a bugfix.

The individual problem results (without considering proof output) show that
19 problems were unsolved,
170 problems were solved by all the systems, and
31 problems were solved by only one system (14 problems were solved by only the two versions
of Vampire, and are counted as unique solutions for Vampire 5.0).
Of the 31 unique solutions,
14 were by Vampire~5.0,
7 by E,
5 by Zipperposition,
3 by cvc5,'
and
2 by Vampire~4.9.
A portfolio (sharing the time between several systems) of these five systems, with 48~s
allocated to each, would solve 474 problems.
...
The Vampires dominated the division again.
It is noteworthy that Vampire~4.9 solved more problems than Vampire~5.0.
The developers believe that the main difference is that Vampire~4.9 has a ``cautiously 
specialized'' schedule~\cite{BCS24-PAAR} of seven branches, while Vampire~5.0 has a single 
monolithic schedule.
The intent was for Vampire~5.0 to have a schedule that might be more general on unseen problems, 
but would show its strength only with longer time limits.
Some new implementation features in Vampire~5.0, e.g., term ordering diagrams~\cite{HC+25}, could 
also have had a positive influence.

A key differentiator between the FOF division systems is their abilities to generate acceptable 
proofs.
There is a notable difference between the ranking by number of acceptable proofs output, and the 
ranking by the number of problems solved.
Vampire~5.0, E, and LisaTT were the only systems to output an acceptable proof for every
problem solved.
It was disappointing that iProver solved so many problems, but failed to output acceptable proofs
for a significant fraction of them, thus pushing iProver down to third place.
Prior to the running of \THIS{} the developer of CSI\_Enigma was close to completing proof 
output, which if completed would have placed the system second.
CSE\_E solved two FEQ problems close to the time limit, but did not finish proof output by 
the time limit. 
Hopefully more systems will consistently produce acceptable proofs in \NEXT{}.

In terms of problems solved (without considering proof output) the ranking is similar to that of 
\LAST{}.
The exceptions were improved performance of CSI\_Enigma over that of CSI\_E in \LAST{}, and
iProver switching positions with CSE\_E.
The improved performance of CSI\_Enigma over CSI\_E can be attributed to the use of the Enigma 
system~\cite{JU17-CICM} as the secondary system instead of E.
Note that all of CSI\_Enigma's proofs took more than 1~s.
The improvement in iProver was due to efficiency improvements and heuristic optimization.
In \LAST{} a clear separation was observed between three groups of systems: ``Vampire~4.9, the 
"hoi polloi", and the "also rans"''.
The performance plots for the FOF division in \THIS{}\footnote{%
\href{https://tptp.org/CASC/30/WWWFiles/ResultsPlots.html\#FOFProblems}{\tt tptp.org/CASC/30/WWWFiles/ResultsPlots.html}} 
show a more progressive separation between the systems, with only Prover9, Connect++, hopCoP, and 
SPASS-SCL being separated into a trailing group.
While the Vampires still stand out ahead, CSI\_Enigma fills the gap between them and the pair
of iProver and E.
...
\hline
Vampire~5.0.1          &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
E~3.5.1                &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
CSI\_PP~1.0            &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
iProver~3.9.4          &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
Drodi~4.1.1            &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
Prover9~2026-6A        &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
LisaST~0.9             &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
FindProof~0.1          &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
SUPr~1.0               &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
mrs~0.2.0              &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
VIP~1.718              &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
Connect++~0.7.2        &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
SATResetCoP~1.0        &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
SPASS-SCL~0.1.1        &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
Zipperp'n 2.1.9999     &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
\hline
\multicolumn{13}{c}{Demonstration division} \\
\hline
\textit{Vampire~5.0}   &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
Prover9~1109a          &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA &  TBA  \\
\hline
\end{tabular}
\end{table*}

The Vampires had the lowest average wall clock times, and correspondingly the highest efficiencies.
Vampire~5.0 had a high SotAC, thanks to it's ability to solve problems that no other systems
solved (see below).
Drodi made the most use of the multiple cores thanks to its parallelisation architecture.
The developer explained: ``Preprocessing is done only once before multiprocessing is started, 
after which the parent process forks as many additional processes as there are cores available. 
The processes share basic information by sharing a small portion of memory that holds a list
of strategies with an associated time slice.
The processes repeatedly take a strategy off the list and run it for the associated time slice.
If a proof is found all processes are stopped, otherwise the process loops to take another
strategy.
This way there is no need to repeat the preprocessing or start a new process for each new 
strategy.''

The performances in the two problem categories are well aligned with the overall performance,
except for SPASS-SCL's poor performance on problems with equality.
The new problems provided different challenges for the systems.
Vampire's monolithic schedule showed its intended ability to solve unseen problems.
E performed worse on the new problems, failing to solve some new logic calculi (the TPTP 
{\tt LCL} domain) problems converted from CNF versions that have been in the TPTP problem 
library for a long time~\cite{RW+23}, hereditary finite set theory (the TPTP {\tt SEV} 
domain) problems~\cite{Bro24}, and software verification (the TPTP {\tt SWX} domain) problems 
encoding arithmetic operations~\cite{MMP24}.
In contrast, Drodi performed relatively well on the new problems, but failed to solve similar 
problems to E.

The individual problem results (without considering proof output) show that
12 problems were unsolved,
no problems were solved by all the systems, and
36 problems were solved by only one system (19 problems were solved by only the two versions of
Vampire, and are counted as unique solutions for Vampire~5.0).
Of the 36 unique solutions,
22 were by Vampire~5.0,
8 by Vampire~4.9,
5 by cvc5,
and
1 by CSI\_Enigma.
...
\label{FNT}
%
Table~\ref{FNTSummary} summarizes the results of the FNT division.
The Vampires came out ahead of the other systems, by a large margin.
As in the FOF division, in terms of the numbers of problems solved, Vampire~4.8 showed only
a small improvement over the \LAST{} winner, Vampire 4.7.
In contrast to the FOF division, the performance plot\footnote{%
\url{https://tptp.org/CASC/29/WWWFiles/ResultsPlots.html\#FNTProblems}}
shows a reversal of the impact of the refreshed strategy schedules in Vampire~4.8, with the
older Vampire~4.7 having the more gradual increase in the CPU time taken over problems solved.
The large numbers of problems solved without a model output by Vampire and iProver is
explained in Section~\ref{TFN}.
In the FNT division Vampire had 31 saturations disallowed, iProver had 27 saturations disallowed,
and iProver had 51 explicit Herbrand models disallowed.
%
\begin{table*}[tbh]
...
\hline
ATP System             & Solns & Solved & Avg  & Sot & WC       & \multicolumn{2}{r}{Core} & FNE & FEQ & Uniq & New  \\
                       & /150  & /150   & WC   & AC  & $\mu$Eff & \multicolumn{2}{r}{usage}&/50  &/100 &  /16 & /10  \\
\hline
Vampire~5.0.1          &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA  \\
iProver~3.9.4          &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA  \\
FMB4J~0.1              &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA  \\
Mace4~2026-6A          &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA  \\
E~3.5.1                &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA  \\
Drodi~4.1.1            &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA  \\
Connect++~0.7.2        &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA  \\
\hline
\multicolumn{12}{c}{Demonstration division} \\
\hline
\textit{Vampire~4.8}   &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA} &  TBA & TBA & TBA & TBA  \\
\hline
\end{tabular}
\end{table*}
%
% There are quite large ranges in average time taken (from E's 9.9~s up to cvc5's 35.1~s),
% SotAC (from E's 0.08 up to Vampire~4.8's 0.33), efficiency (from cvc5's 163 up to Vampire~4.8's
% 513), and core usage (from cvc5's 0.9 up to Vampire~4.7's 4.7).
cvc5's low core usage led to its high average time taken and low efficiency.
...
only 2 problems were unsolved,
11 problems were solved by all the systems, and
26 problems were solved by only one system (21 problems were solved by only the two versions of
Vampire, and are counted as a unique solutions for Vampire~4.8).
Of the 26 unique solutions,
23 were by Vampire~4.8,
and
3 by iProver.
...
%
The ongoing research and development of model finding, in the context of the TPTP World, and
its impact on CASC, is discussed in Section~\ref{ModelFinding}.
The conclusion that most affects CASC is that the FNT division will go on hiatus for \NEXT{}.
%--------------------------------------------------------------------------------------------------
...
Vampire~5.0 won, with an improvement over Vampire~4.9.
The recent advances in Vampire's UEQ calculus and implementation~\cite{HKV25,HC+25} have
contributed significantly to Vampire's strong performance.
Twee~2.6.0 solved the same number of problems as Vampire~4.9, indicating improvement since
Twee~2.5.0, which was convincingly beaten by Vampire~4.9 in \LAST{}.
The Twee developer explained: ``this year's version has fixes for some completeness bugs, 
which allows some slightly more aggressive redundancy criteria to be used''.
Only CSE\_E and iProver failed to output acceptable proofs for all problems solved.
In the case of iProver this was quite severe, moving it from a possible fourth place down to
the bottom of the competition division (see Section~\ref{Acceptable} for more details).
...
ATP System             & Solns & Solved & Avg  & Sot & WC       & \multicolumn{2}{r}{Core} & Uniq & New & Gr'ty \\
                       & /400  & /400   & WC   & AC  & $\mu$Eff & \multicolumn{2}{r}{usage}&  /48 & /79 &       \\
\hline
Vampire~5.0.1          &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA}  & TBA & TBA &  TBA  \\
Twee~2.7               &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA}  & TBA & TBA &  TBA  \\
CSI\_PP~1.0            &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA}  & TBA & TBA &  TBA  \\
E~3.5.1                &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA}  & TBA & TBA &  TBA  \\
FindProof~0.1          &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA}  & TBA & TBA &  TBA  \\
Prover9~2026-6A        &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA}  & TBA & TBA &  TBA  \\
iProver~3.9.4          &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA}  & TBA & TBA &  TBA  \\
Drodi~4.1.1            &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA}  & TBA & TBA &  TBA  \\
mrs~0.2.0              &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA}  & TBA & TBA &  TBA  \\
\hline
\multicolumn{11}{c}{Demonstration division} \\
\hline
\textit{Vampire~5.0}   &  TBA  &  TBA   &  TBA&  TBA & TBA      & TBA       & {\tiny /TBA}  & TBA & TBA &  TBA  \\
\hline
\end{tabular}
\end{table}

The SotAC, efficiency, and core usage values were roughly aligned with the division ranking, 
with the exceptions of iProver's higher SotAC, and Drodi's higher efficiency and core usage.
It is noteworthy that compared to the other divisions a higher fraction of problems were solved
with a core usage greater than 1.0.
The exception was Toma, which makes no use of the multiple cores (the 19 results that appear
to use multiple cores are an artifact of the competition measurement harness).
Over the other systems, an average of 90\% of the problems were solved with multiple cores used.
This is in contrast to, e.g., the FOF division, where the average (excluding the systems that did
not try to use multiple cores) is 76\%.

The individual problem results (without considering proof output) show that
4 problems were unsolved,
59 problems were solved by all the systems, and
17 problems were solved by only one system (4 problems were solved by only the two versions
of Vampire, and are counted as unique solutions for Vampire~5.0).
Of the 17 unique solutions,
7 were by Vampire~5.0,
6 by Twee~2.6.0,
and
5 by Vampire~4.9.
A portfolio of Vampire~5.0, Twee, and E, with 80~s allocated to each, would solve 285 problems.
Although there were no problems that only E solved, its ability to solve problems quickly makes
it a useful contributor to a portfolio.
For the suggested portfolio there are 11 problems that only E can solve within 80~s.
...
The CASC-J12 report noted the benefits of moving towards a ``verified track'' in CASC, and in 
\LAST{} first steps were taken, to check three aspects of proofs:
\begin{packed_enumerate}
\item Solutions must be written in the TPTP language~\cite{Sut23-IGPL}.
\item Solutions must be written in the TPTP formats for derivations~\cite{SS+06}, tableaux, and 
      models~\cite{SS+23-LPAR}.
\item Solutions must be structurally correct, e.g., derivations must be acyclic, finite models must
      have domains.
\end{packed_enumerate}
The impact was interesting, and it was clear that the ATP systems needed to further improve their
proof output.
In \THIS{} the TPTP4X parser~\cite{Sut10} and the GDV proof verifier~\cite{Sut06} were made 
available to the systems' developers so that they could check their proofs.
Despite this, many proofs were still syntactically or structurally flawed.
The \THIS{} panel ruled that the documentation of the TPTP proof format was inadequate, which
meant many of those proofs were eventually counted.

What got better?

There are 482 proofs that failed structural checking.
The reasons for failure are varied: 
12 because a refutation has a root node that is not {\tt \$false},
30 because there are duplicate formula names in the proof,
91 because an introduced definition is incorrectly documented,
172 because an inference step references a non-existent parent,
and
177 because of syntax errors in the formulae.
As can be seen from Table~\ref{FailedVerification}, the impact was greatest on iProver, which
had all 91 of the cases of incorrect definition documentation, and 147 proofs that referenced 
non-existent parents.
These seem like easy fixes, which bodes well for iProver in \NEXT{}.
Drodi was also strongly impacted, with 120 proofs containing formulae with syntax errors.
The developer agreed: ``all those 120 problems had bad solutions'', and it is expected that this 
will be fixed ready for \NEXT{}.

Discussion of granularity:
It was observed that many of the ATP systems' proofs include ``inference steps'' that are calls 
to SAT/ATP/SMT systems, which infer {\tt \$false} to end the ATP system's refutation.
Drodi calls an internally implemented SAT system.
E calls the SAT system PicoSAT~\cite{Bie08}.
iProver calls the SAT system MiniSat~\cite{ES04} and the SMT system Z3~\cite{dMB08}.
Leo-III calls the ATP system E.
Vampire~5.0 calls the SAT systems MiniSat and CaDiCal~\cite{BF+24} (the choice depends on the 
proving context, as one of the developers explained: ``MiniSat has a quick-and-dirty flavour 
in our experience, CaDiCaL "thinks" a bit harder''), and the SMT system Z3. 
In most cases these steps in the ATP systems' proofs have a very long parent list.
The most parents listed in the SAT/ATP/SMT systems' steps were in
E's refutation for {\tt GEG019\verb|^|1} that ends with a PicoSAT step with 114 parents,
Leo-III's refutation for {\tt SWW474\verb|^|3} that ends with an E step with 950 parents,
Drodi's refutation for {\tt ALG104+1} that ends with a SAT step with 1219 parents,
Vampire~5.0's refutation for {\tt SWW593\_2} that ends with a Z3 step with 5617 parents,
and
iProver's refutation for {\tt NUM862+1} that ends with a Z3 step with 72367 parents!
Discussions with the systems' developers revealed that these steps are typically reported with
the parent list containing all the formulae that were sent to the SAT/ATP/SMT system, and it is
not known which of them were used by the SAT/ATP/SMT system to infer {\tt \$false}.
...
For examples, 
Leo-III's use of E needed only 14 of the 950 reported parents,
Drodi's use of it's SAT solver needed only 663 of the 1219 reported parents,
and
Vampire~5.0's use of either MiniSat or CaDiCaL (the proof does not indicate which) needed only
194 of the 5617 parents.
iProver's use of Z3 defied analysis because of the size of the parent list!
Only E and Drodi make an effort to extract the formulae that were actually used by the 
SAT/ATP/SMT system.
E precisely extracts the unsatisfiable core found by PicoSAT, and thus the parent list contains 
only the formulae that were used.
Drodi makes an effort to minimize the parent list from its SAT solver steps, but the developer 
admitted: ``I cannot guarantee that this set is the minimum possible, but I used the best algorithm 
I could design to minimize it''.

In an effort to further improve the quality of ATP systems'proofs, \NEXT{} will require the 
systems to aim to report only the parents that are used in each inference step of a proof. 
It is clear that the systems can only ``aim to'' comply, because precisely extracting the set of 
parents that were used by a SAT/ATP/SMT system might not be possible in all cases.
From the organizational side, checking conformance to that requirement might not be possible in 
all cases. 
We'll all just do the best we can.
...
ATP system's proof.
An interesting two-level example starts with Leo-III's refutation for {\tt GEG019\verb|^|1} that 
ends with an E step with nine parents.
It is quite possible that the E step is the one mentioned above, that ends with a PicoSAT step 
with 114 parents.
Whether or not Leo-III should be expected to burrow down to expose the PicoSAT proofs within 
E proofs is a matter for debate -- it will be the focus of the TPTP Tea Party at IJCAR in 2026.
Grafting in the proofs of subsystems might be required in future CASCs.
...
ProoVer is decribed in Section~\ref{ProoVer}.
...
\begin{packed_itemize}
\item Only two installation package will be allowed for each entry -- one for theorem proving/
      unsatisfiability checking, and one for coutermodel finding/satisfiability checking.
\item Proof checking will be extended to include:
      ...
      \end{packed_itemize}
\item Syntax checking of proofs will be done at runtime during the competition.
