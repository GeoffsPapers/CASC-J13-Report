...
\end{frontmatter}
%--------------------------------------------------------------------------------------------------
\paragraph{Author contributions:} With the following caveats, the paper was written by 
the paper author.
...
Section~\ref{ProoVer} was written in collaboration with Julie Cailler and Simon Guilloud.
...
13th International Joint Conference on Automated Reasoning (IJCAR 2026)\footnote{%
...
\item Inference steps in proofs were expected to list only parents that are used in the inference.
...
System descriptions are in the competition proceedings~\cite{Sut26-CASC-J13} and on the \THIS{}
web site.
...
In \THIS{} a total of 1618 CPU hours were used in 353 wall clock hours, i.e., 5.9 hours spread
...
\item The TPTP uses system performance data in the Thousands of Solutions from Theorem Provers
      ...
      Using problems that are solved by all or none of the competition systems reduces the
      differentiation between the systems in the competition, but does not affect the competition 
      rankings.
      It does have the benefit of placing the systems' performances in the context of the 
      state-of-the-art in ATP.
\end{packed_itemize}
...
The time limit is chosen within the range allowed according to the judgement of the
...
using a system registration form.
...
\item Proofs must be syntactically correct in TPTP format~\cite{Sut07-CSR}.
      In \THIS{} syntax was checked using TPTP4X~\cite{Sut10}.
\item Proofs must be structurally correct:
      ...
      \item Proofs that make \smalltt{assumption}s must propagate and eventually discharge the
            assumptions.
      \end{packed_itemize}
      In \THIS{} structure was checked using GDV~\cite{SB05}.
...
site.\footnote{%
\href{https://tptp.org/CASC/J13/WWWFiles/Results.html}{\tt tptp.org/CASC/30/WWWFiles/Results.html}}

The structural checking of proofs in the competition theorem proving divisions was new in \LAST{}, 
and many systems' proofs had structural flaws.
In \THIS{}, some developers declared in advance that their systems' proofs would not be acceptable, 
and their numbers of acceptable solutions output is set to ``-'' rather than 0 in the results 
tables.
Discussion of the progress in proof output from \LAST{} to \THIS{} is given in 
...
The 23 missing proofs from Leo-III are interesting: they come from Leo-III using bracketed
...
Analysis revealed that their higher granularities came from inferences that were calls to the 
E system.
...
Vampire~5.0.1 improved over Vampire~5.0 because the old \smalltt{avatar\_sat\_refutation} 
inferences have been replaced by the CaDiCaL refutation steps.
The main motivation for this work was the ongoing proof-checking efforts in Vampire.
The developer responsible for this change explained: ``it wasn't so hard with CaDiCaL's 
spiffy new Tracer API. You hook your own Tracer subclass into CaDiCaL and it tells you when 
it learns new clauses.''
% This made it possible to extend the Vampire refutation with the CaDiCaL refutation.
The developer admitted that although: ``the dreaded "avatar sat refutation" is no more. The 
even-more-dreaded "avatar smt refutation" remains at large.'', which is because that rule
is used only for proofs involving arithmetic, as in the TFA division of \LAST{}.
...
from the previous version of the system to the new version.
One of the Vampire developers explained: ``most of the improvement between 5.0 and 5.0.1 comes 
from the addition of the neurally guided clause-selection strategies~\cite{Sud25} \ldots~Since 
we solve many hard (previously unsolved) problems with the neurally guided strategies, I think 
a key was to submit the new version of Vampire early, for the problem rating, so that these 
problems would become eligible for CASC and could showcase the improvement.''
...
of the systems, but the grouping has changed since \LAST{}.
...
This year the grouping is even less pronounced, but is still evident:
...
% equality), and VIP.
There are no salient technical features of the systems in each group. 
The difference seems to be in years of developmental experience.
...
lessons were learned from the \LAST{}.
...
In the FOF division there were five absolute newcomers: LisaST, FindProof, SUPr, mrs, and VIP.
Additionally there were three new variants of existing systems: CSI++, Prover9~2026-6A, and
SATResetCoP.
...
This can be seen in the results plot, with E having a sharper upwards swing than the Vampires.
...
Drodi also had high core usage, and solved 89 problems with a raw core usage less than 1.0.
...
% and
% 3 by Vampire~5.0.
Between them, the two Vampires solved a superset of the union of the problems solved by 
the other systems.
...
The winner from CASC-29 in 2023 was entered as the previous winner.
...
The key to the Vampires' success was strategy scheduling that tries to saturate the clause
...
It's value is well appreciated by the Vampire developers, one of whom said: ``tgt-ground 
{\em [the goal directed technique]} is definitely worth having. We are actually considering 
baking it into the default strategy.''
...
Twee~2.7               &  286  &  286   & 22.4 & 0.24 & 225     & 6.4       & {\tiny /273} &   2 &   7 &  \bb{2.0}  \\
...
The granularity values are reasonably low, with Twee and Prover9 having optimal values.
...
\textbf{FindProof}~\cite{Mur26} is an equational theorem prover based on unfailing (ordered) 
Knuth–Bendix completion~\cite{BDP89}; the completion loop and critical-pair selection follow 
Waldmeister's design~\cite{HL02}, with selection and redundancy techniques from Twee, Vampire, 
and E: ground joinability, connectedness, goal-directed and age–weight selection. 
Orderings are KBO and LPO with generated precedences. 
First-order input is clausified with definitional CNF and translated to equational and Horn form; 
goal-subterm naming supports goal-directed search, and existential goals are closed by narrowing. 
Proofs are emitted as TSTP derivations and checked internally before output.
The implementation is one self-contained C program with no dependencies, using flatterms with 
perfect discrimination trees. 
In the competition about forty strategy configurations ran concurrently, each with the full wall 
clock time limit. 
The same engine ships as a Wolfram Language paclet: TFindProof resolves theories through 
AxiomaticTheory and returns proof objects compatible with FindEquationalProof. 
Development is AI-assisted: ports, dissection against reference implementations, and regression 
testing are done largely by language model agents, with changes gated on byte-identical trajectory 
pins.
CASC drives development: fresh problem selection exposed overfitting to previous years' problems, 
and comparison with Vampire, Twee, and E sets priorities. 
Planned work extends coverage from UEQ and FOF to the typed, higher-order, satisfiability, and 
large-theory divisions, and eventually all TPTP divisions.
...
In the commentary of the results in the THF, FOF, and UEQ divisions (Sections~\ref{THF}, \ref{FOF}, 
and~\ref{UEQ}), the extent to which the systems output acceptable proofs is discussed.
For newcomers to CASC imperfect proof output might be forgiven.
...
that were entered in both.

%--------------------------------------------------------------------------------------------------
\subsection{Requirements}
\label{Requirements}

Prior to \LAST{} the opinion of the CASC panel was the only adjudication of whether systems' 
proofs were acceptable. 
These guidelines were set for CASC-J12~\cite{Sut24-CASC-J12}:
\begin{packed_enumerate}
\item[1.] Proofs must be complete, starting at formulae from the problem, and ending at the 
      conjecture (for axiomatic proofs) or \smalltt{\$false} (for proofs by contradiction, 
      e.g.,~CNF refutations).
...
in CASC.
An incremental approach was adopted, and the following requirements were added in \LAST{}:
\begin{packed_enumerate}
...
\end{packed_enumerate}
The TPTP4X parser and GDV proof verifier were made available to the 
systems' developers so that they could check their proofs in using the same tools as were used for 
the competition evaluation (see Section~\ref{Evaluation}).
...
their proofs~\cite{Sut26-CASC}.
A particular phenomenon observed was that many of the ATP systems' proofs included ``inference 
steps'' that were calls to SAT/ATP/SMT systems that inferred {\tt \$false} to end the refutation.
These final calls to external systems were typically reported with all the clauses sent to the 
external system listed as parents -- the most extreme case listed 72367 parent clauses that were 
sent to the external Z3 system!
...
%--------------------------------------------------------------------------------------------------
\subsection{Progress from \LAST{} to \THIS{}}
\label{Progress}

Table~\ref{ProofProgress} provides data comparing the progress in acceptable proof production
between \LAST{} and \THIS{}, for systems that had versions entered in both competitions.
Loss measures what percentage of the problems solved did not have acceptable proof output:
\[
Loss\,\text{\em \%} = 
\dfrac{Solved - Solns}{Solved}
\]
A reduction in Loss, indicated by a negative $\Delta$~Loss, directly indicates improvement.

The Progress column measures that improvement in the context of the numbers of problems
solved and the numbers of problems in the divisions:
\[
Progress = 
\begin{cases}
\text{if 0\% Loss both years} , & \text{\em \LAST{}}\!\left(\dfrac{Solns}{Problems}\right) - \text{\em \THIS{}}\!\left(\dfrac{Solns}{Problems}\right) \\[10pt]
\text{otherwise}              , & \text{\em \LAST{}}\!\left(\dfrac{Solved - Solns}{Problems}\right) - \text{\em \THIS{}}\!\left(\dfrac{Solved - Solns}{Problems}\right)
\end{cases}
\]
The Progress values for systems that had 0\% loss in both years, when $Solved = Solns$ and 
indicated by a `-' in the $\Delta$~Loss column, are not directly comparable with the others, 
but do measure progress in acceptable proof production.

There are very definite signs of progress, partiularly in the FOF division.
Drodi's improvement is salient, with iProver also having significant improvement (again in the
UEQ division).
E is noteworthy for having consistently minimal loss, coupled with improved performance.
As an overall measure, in \LAST{}, 459 of the 3961 proofs from the THF, FOF, and UEQ divisions, 
i.e.~11.5\%, had syntax or structural flaws.
...
\hline
Division    & \bb{\LAST{}}&Problems&    &         & \bb{\THIS{}}&Problems&   &        & $\Delta$ Loss &Progress \\
...
A comparison of the granularity values between \LAST{} and \THIS{} would be ``unfair'', because
the measure was not part of the \LAST{} design, and thus developers had no motivation to minimise
granularity.
The granularity values reported in Sections~\ref{THF}, \ref{FOF}, and~\ref{UEQ} provide
some indication of the situation in \THIS{}.
As a counter point to the maximal value of 72367 clauses sent to Z3 in \LAST{}, the maximal value 
in \THIS{} was 12651.
...
Analysis after \THIS{} revealed that many proofs had ill-formed documentation in their 
\smalltt{introduced{}} and \smalltt{inference()} records.
The \THIS{} panel ruled that the documentation of the required format for these records was 
inadequate, and thus those flawed proofs were counted.
The documentation has been improved for these two points and more, and more precise requirements 
have became possible.
These are given in requirements 9', 10', 14, and 15 below.

Discussions during the IJCAR 2026 conference that hosted \THIS{} led to the conclusion that 
...
The SAT/ATP/SMT systems typically build a proof of their own, and that proof structure is often not 
shown in the ATP system's proof.
It would be preferable for the SAT/ATP/SMT system's proof to be exposed by being grafted onto the
ATP system's proof (as done in Vampire~5.0.1's use of CaDiCaL - see Section~\ref{THF}).
Grafting on the proofs of subsystems will be encouraged in \NEXT{}, specified in requirement
13 below.

In addition to these reactive changes to observations from \THIS{}, two further requirements
will be added.
Firstly, as another incremental step towards a verified track in CASC, leaves of proofs will
be required to be copied from the problem, modulo variable renaming.
To that end requirement 12 below will be added.
This is in agreement with the requirements of the ProoVer competition described in 
Section~\ref{ProoVer}.
Secondly, users of ATP systems can reasonably expect proofs to not use TPTP language features
that are not supported by the language used for the problem.
For example, proofs for FOF problems should not use THF higher-order features, and if that
restriction is observed then the proofs can be expected to be written in FOF.
To that end requirement 16 below will be added.
...
\item[12.] Leaf annotated formulae of proofs must be alpha-equivalent to the corresponding named
      annotated formula in the problem.
\item[13.] Proofs should graft in subproofs from external systems.
\item[14.] Requirements for the \smalltt{introduced()} records of symbol definitions, 
      introduced tautologies, and theory axioms, will be provided.
\item[15.] Requirements for the \smalltt{inference()} records of Skolemization steps will be 
provided.
\item[16.] Proofs should not use a TPTP language that is more expressive than that of the problem.
\end{packed_enumerate}
The BNF-based parser~\cite{VS06}, the TPTP4X parser, and an improved version of GDV will be 
made available to the systems' developers so that they can check their proofs.
This can be done online in SystemOnTSTP.\footnote{%
...
%--------------------------------------------------------------------------------------------------
\subsection{Acceptable Models}
\label{AcceptableModels}

Up to and including \THIS{}, the CASC panel was the only adjudication of whether systems' 
models were acceptable. 
The following guidelines were set for \THIS{} (see Section~\ref{Evaluation}):
\begin{quote}
Models must be complete, documenting the domain and symbol maps. 
The domain and symbol maps may be specified by explicit ground lists (of mappings), or by 
any clear, terminating algorithm. 
Saturations are acceptable if their models are a subset of the models of the problem formulae. 
Use of the TPTP format for interpretations is encouraged.
\end{quote}
For \NEXT{} the requirements will be made more precise:
\begin{packed_enumerate}
\item[1.] Models must be written as annotated formulae in TPTP syntax.
\item[2.] Annotated formulae in a model must be uniquely named.
\item[3.] Models must be complete, documenting the domain and symbol maps.
\item[4.] The domain and symbol maps may be specified by explicit ground lists (of mappings), 
          or by any clear, terminating algorithm.
\item[5.] Models must show all and only relevant information.
\item[6.] Use of the TPTP format for interpretations is encouraged.
\end{packed_enumerate}
Models will syntax checked using the BNF-based parser and the TPTP4X parser.
...
verifiers, i.e.,~systems that verify whether a proof is correct wrt its conjecture. 
...
ProoVer 2026 was organized by Julie Cailler and Simon Guilloud, and overseen by the same panel 
as \THIS{}: Cl\'audia Nalon, Aart Middeldorp, and Martina Seidl.
An overview of the competition is given here.
Full details of the competition design and the entrant systems are given in the 
ProoVer~2026 proceedings~\cite{CG26}. 

A ProoVer~2026 entrant is a proof verifier that is given an untyped first-order (FOF) TPTP 
problem and a corresponding refutation proof~\cite{SS+06}, and decides whether or not 
the refutation proof is valid.
A refutation proof consists of a sequence of syntactically correct TSTP inference steps ending
at \texttt{\$false}.
Most inference steps could be discharged by an external ATP system, but the verifiers had to
verify a small set of specified rules internally -- Skolemization, negation of the conjecture, 
and correspondence between the problem file and the proof file.
A wall clock time limit of 60~s was imposed on each verification attempt.
The systems had to report their decision using the SZS ontology, one of VerifiedGood (VSG), 
VerifiedBad (VSB), or Unknown (UNK).
Any other result, e.g., a timeout, was treated as UNK.

The systems were ranked according to the following grading scheme:
\begin{itemize}
...
\end{itemize}
The problem set originally had $50$ good proofs and $50$ bad (aka ``evil'') ones that had 
invalid inferences or other bad features.
However, 9 proofs were later removed as ambiguous or non-compliant, leaving 44~good and
47~bad proofs that were used for the ranking. 
The possible score therefore ranged from 138 to -514.
...
Table~\ref{ProoVerSummary} lists the entrants and the results. 
GAPT~2.20 won the competition with 111 points.
Since at least one student was involved in the development of GAPT~2.20, it also won the 
\textit{ProoVer 2026 Best Student Proof Checker} award.
VaLeaDate~0.1 won the \emph{ProoVer 2026 Safest Proof Checker} award: it tied with N\"orgler 
for the most Bad proofs correctly identified (46 out of 47), but it did so without any 
VerifiedGood verdicts on a Bad proof.
On the Good proofs side, PyCheck~0.1 performed best, correctly identifying 37 Good proofs against 
only two misclassified as Bad.
...
\begin{table}[tbhp]
\caption{ProoVer~2026 summary}
\label{ProoVerSummary}
\begin{tabular}{ll|rrrr|rrrr|r}
\hline
                  &          
    &\multicolumn{4}{c|}{44 Good proofs}& \multicolumn{4}{c|}{47 Bad proofs} & Score \\
 \hline
Verifier          & Entrants 
    & VSG & VSB & UNK &   /44 & VSB & VSG & UNK & /94 & /138 \\
\hline
GAPT~2.20         & Fabian Achammer \& Martin Riener 
    &  34 &   5 &   5 &    29 &  41 &   0 &   6 &  82 &  \bb{111} \\
VaLeaDate~0.1     & Jonas Bodingbauer \& Laura Kovacs 
    &  24 &  18 &   2 &     6 &  \bb{46} &   0 &   1 &  \bb{92} &   98 \\
N\"orgler~1.1     & Alexander Steen et al.            
    &  27 &  16 &   1 &    11 &  \bb{46} &   1 &   0 &  82 &   93 \\
ProofCheck~1.0    & Jeff Machado \& Larry Lesyna     
    &  31 &  10 &   3 &    21 &  42 &   3 &   2 &  54 &   75 \\
ProofGuard~1.0    & Matthew Farah \& Alicia Marinache 
    &  30 &  11 &   3 &    19 &  40 &   5 &   2 &  30 &   49 \\
mrs-proover~0.2.0 & Olivier Roland
    &  31 &  12 &   1 &    19 &  39 &   5 &   3 &  28 &   47 \\
PyCheck~0.1       & Stephan Schulz                    
    &  \bb{37} &   \bb{2} &   5 &    \bb{35} &  28 &   5 &  14 &   6 &   41 \\
CheckProof~0.1    & Nik Murzin
    &  28 &  11 &   5 &    17 &  25 &  17 &   5 &-120 & -103 \\
\hline
\multicolumn{9}{c}{\textit{Demonstration division}} \\
\hline
GDV~2.0           & Geoff Sutcliffe
    &  35 &   5 &   4 &    30 &   8 &   5 &  34 & -34 &   -4 \\
GDV-LP~2.0        & Fr\'ed\'eric Blanqui \& Geoff Sutcliffe
    &  33 &   5 &   6 &    28 &   8 &   5 &  34 & -34 &   -6 \\
\hline
\end{tabular}
\end{table}
...
\THIS{} fulfilled its objectives by evaluating the relative capabilities of ATP systems,
and stimulating development and interest in ATP.
The highlight of \THIS{} was the large number of newcomers who provided counterpoints for the
systems that have been enetered for many years.
The lowlight was the number of syntactically and structurally flawed proofs, which hopefully will
motivate developers to pay more attention to this aspect of their ATP systems.
...
of the conference that provides opportunities for developers, users, and observers to interact.

While the design of CASC is mature and stable, each year's experiences lead to ideas for changes
and improvements.
Changes planned for \NEXT{} are:
\begin{packed_itemize}
\item Only two installation packages will be allowed for each entry -- one for theorem 
      proving/unsatisfiability checking, and one for countermodel finding/satisfiability checking.
...
\item All solutions -- proofs and models -- must be in TPTP syntax, error-free according to 
      the BNF-based and TPTP4X parsers.
\item Proof checking will be extended as described in Section~\ref{FutureProofs}.
\item No sample proofs will be requested, to avoid any mistaken belief that one sample proof
      approved by the panel implies that all proofs will be acceptable.
\end{packed_itemize}
