...
two octa-core Intel(R) Xeon(R) CPU E5-2620~v4 @~2.10GHz CPUs, with hyperthreading (two threads 
per core),
256GiB memory,
...
These flaws in the proof are quite simple, e.g., many of them have duplicate formulae names, in 
some cases duplicate formulae, and others have the SZS \smalltt{status(esa)} when negating 
the conjecture, while it should be \smalltt{status(cth)}.
...
quantifiers as terms that can be applied, e.g., \smalltt{((!) @ \$i)}, which might be 
acceptable as a higher-order notion, but is not permitted in the TPTP syntax.
...
Zipperp'n~2.1.9999     &    0  &  347   & 16.7 & 0.31 & 334      & 5.7       & {\tiny /268} &  67 & 280 &   1 &  29 &    -  \\
\hline
...
Zipperposition had the highest average time taken, but made the bext use of the multiple cores.
...
This is the first CASC in which the new granularity data has been provided, motivated by the
very last numbers of parents listed in individual inferences of \LAST{}.
The report on \LAST{} noted that final calls to external ATP systems were typically reported 
with all the clauses sent to the external system listed as parents.
The most extreme case listed 72367 parent clauses that were sent to the external Z3 system.
The granularity figures for the THF division (recall, they are the average maximal number of
parents in inferences over problems solved with acceptable proof output) are not extreme.
Deeper analysis revealed that LEO-II and Leo-III often had inference with many parents, by
virtue of calls to the E system.

E had solved the most TNE problems, largely because it uniquely solved 19 graph theory (\smalltt{GRA}) problems.
These problems have been in the TPTP problem library since release v3.6.0 from 2008.
E's developer explained ASKED STEPHAN.
...
As in \LAST{}, the Vampires dominated the division again, but this year there is clear progress
from Vampire~5.0 to Vampire~5.0.1.
The developers explained: ASKED MARTIN.
...
This year the Vampires are out front as usual, then there is a group of CSI++, E, and iProver,
then a middle group of Drodi, LisaST, Prover9~2026-6A, and Zipperposition, a lower group of
FindProof, mrs, Prover9---1109a, and SUPr, and finally the bottom group of Connect++, 
SATResetCoP, SPASS-SCL (noting that SPASS-SCL cannot attempt problems with equality), and 
VIP.
...
CASC clear provides opportunities for system developers to learn from others!
...
lessons were learned from the \LAST{} results.
However, there is still room for improvement.
The unacceptable proofs often had simple syntax mistakes, that can be easily fixed, e.g.,
illegal formula names, missing brackets around binary formulae.
Some other flaws were deeper, e.g., missing formulae.
In \NEXT{} the requirements for proofs to be considered ``acceptable'' will be raised, as 
...
Aditionally there were four new variants of existing systems: CSI++, Prover9~2026-6A, LisaST, and
SATResetCoP.
...
The results show great improvement over Prover9~1109a, which has been the fixed point in the
CASC FOF division since CASC-J6 in 2013.
The improvement was not only in problems solved, but also TPTP format proof output.
The developers explained: ASKED JEFF AND LARRY.
...
ConnectPP Huge time - check! No core usage, none fast.
VIP Huge time large startup time? No core usage, none fast.
...
and
3 by Vampire~5.0.
That means that between them, the two Vampires solved a superset of union of the problems solved 
by other the systems.
...
iProver 64 with syntax errors, 42 duplicate formula name, 22 unquantified variables. Needs only simple fixes.
Vampire 4.8 92 with syntax errors, simple redeclaration of \$i. Historical error, could be easily fixed.
...
the problems solved by other the systems.
...
\item All solutions (proofs and models) must be in TPTP sytax, error-free according to TPTP4X.
