...
As can be seen, that target of 50\% new problems was not met, but there were enough new problems 
...
problems quickly and very few slowly~\cite{SS01,Sut24}.
...
E had solved the most TNE problems, largely because it uniquely solved 19 graph theory 
...
with all the clauses sent to the external system listed as parents, with the most extreme 
case listing 72367 parent clauses that were sent to the external Z3 system.
...
group of FindProof, mrs, Prover9---1109a, and SUPr, and finally the bottom group of Connect++, 
...
LisaST suffered the most in this regard.
...
LisaST used multiple cores for most of the problems it solved ASKED SIMON.
Drodi also had high core usage, but solved 89 problems in less than 1~s when multiple cores
would be expected to be used ASKED OSCAR.
...
The granularity values are very reasonble for most of the systems.
The best came from Connect++, LisaST, Prover9~2026-6A, and VIP, which all had a granularity of 2.0. 
That means those systems' inference had an average of maximally two parents per inference, making 
the proof as fine-grained as can be expected.
...
except for LisaST's stronger performance and SPASS-SCL's zero performance on problems with 
equality.
The new problems provided consistent challenges for the systems, with about 20\% of the problems
solved being new problems, except for Prover9~2026-6A where a slighty-lower 17\% of the problems
it solved were new.
Does this suggest some over tuning - ASKED JEFF AND LARRY.
...
The winner from CASC-29 in 2003 was enetered as the previuous winner.
Table~\ref{FNTSummary} summarizes the results of the FNT division.
Vampire~5.0.1 came out ahead of the other systems, by a large margin, surpssing Vampire~4.8
...
The ratios of saturations to (finite) models ouput was as follows:
Vampire~5.0.1 - 59:84, 
Vampire~4.8 - 36:91,
iProver - 6:65,
FMB4J 0:51,
...
The goal directed technique pioneered by Twee~\cite{TBA}
FindProof, mrs, and Prover9 failed to output acceptable proofs for all problems solved.
FindProof simply did not output any proof for 18 problems, mrs output one proof with syntax 
errors, and for Prover9, as the developers put it when fixing the ourput for their next
release: ``The bug is an amusing one. Prover9 still registers Otter’s legacy infix v (disjunction) 
as a parse rule, so when a TPTP problem legitimately uses v as a binary functor, the printer 
rendered the legacy in infix notation''.
