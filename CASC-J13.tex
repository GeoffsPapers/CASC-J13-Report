...
\item The \textit{granularity} measures the extent to which inference steps combine multiple 
      ...
      overall granularity is the average over the problems it solves with acceptable proof output.
...
very last numbers of parents listed in individual inferences of \LAST{}.~\cite{}
See Section~\ref{Acceptable} for some discussion.
The granularity figures are not extreme (recall, they are the average maximal number of
parents in inferences over problems solved with acceptable proof output).
...
Drodi also had high core usage, and solved 89 problems in less than 1~s.
...
The granularity values are very reasonable for most of the systems.
...
iProver had eight prof with a granularity of over 1000, three of which were over 10000, with
the maximal being 12651.
As was the case in \LAST{} iProver's use of Z3 to finish a refutation is the common culprit,
seen in iProver's inference rule \smalltt{smt\_impl\_just}.
SATResetCoP had an outlier in it's proof of \smalltt{CSR058+3}, with a
\smalltt{satresetcop\_premises} inference with 8006 parents.
% , average 531, median 44.
\smalltt{satresetcop\_premises} builds a conjunction of the axioms and negated conjecture. 
Beyond the proof of \smalltt{CSR058+3} SATResetCoP had mostly much lower granularity values.
ASKED MARTIN WHAT'S HAPPENING.
...
being new problems, and for all the systems about 20\% of the problems solved were new problems, 
except for Prover9~2026-6A for which 17.5\% of the problems solved were new problems.
...
Several other systems have adopted the idea: E, TBA.
...
errors, and for Prover9, as the developers put it when fixing the output for their next
release: ``The bug is an amusing one. Prover9 still registers Otter’s legacy infix v (disjunction) 
as a parse rule, so when a TPTP problem legitimately uses \smalltt{v} as a binary functor, the 
printer rendered the legacy in infix notation''.
...
acceptable, based on the following guidelines~\cite{Sut24-CASC-J12}:
...
The CASC-J12 report noted~\cite{Sut25-CASC} the benefits of moving towards a ``verified track'' 
in CASC.
An incremental approach was adopted, and in \LAST{} the following requirements were added.
\begin{packed_enumerate}
...
\end{packed_enumerate}
TPTP4X parser~\cite{Sut10} and the GDV proof verifier~\cite{Sut06} were made 
...
proof output~\cite{Sut26-CASC}.
There were two particular points of weakness observed in the \LAST{} proofs.
...
of flaws.
Second, and more fundamentally, it was observed that many of the ATP systems' proofs included
``inference steps'' that were calls to SAT/ATP/SMT systems, which inferred {\tt \$false} to 
end the ATP system's refutation.
...
\item[12.] Proofs that make \smalltt{assumption}s must propagate and eventually discharge the 
      assumptions.
\end{packed_enumerate}
Additionally, granularity measure was added to the results to track the maximal numbers of parents
in the inference steps of proofs.
...
all cases. 
It is clear that the systems can only ``aim to'' comply, leading to a change to the requirement.
...
ATP system's proof.
Grafting in the proofs of subsystems will be required in future CASCs.
...
The documentation was improved after the competition, and more specific requirements became
possible.
...
\end{packed_enumerate}
A BNF-based parser~\cite{SS+06}, the TPTP4X parser and an improved version of GDV will be made
available to the systems' developers so that they can check their proofs.
...
{\em of} a theorem (that was a conjecture), but rather a proof that a conjecture {\em is} a 
theorem.
